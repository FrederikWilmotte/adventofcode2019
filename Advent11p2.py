...
def paintingRobot(intcode):
    ...
    maxRij = 2000
    maxKolom = 2000
    panel = [[" " for x in range(maxRij)] for y in range(maxKolom)]
    panel[rij][kolom] = "X"
    signalsIn = []
    if panel[rij][kolom] == " ":
        signalsIn.append(0)
    ...
    if len(signalsOut) > 0:
        ...
        (rijU,rijD,kolomL,kolomR) = bepaalOmtrek(rij,kolom,rijU,rijD,kolomL,kolomR)
    while status == "waiting":
        ...
        if len(signalsOut) > 0:
            ...
            (direction,rij,kolom) = facingDirection(direction,signalsOut[1],rij,kolom)
            (rijU,rijD,kolomL,kolomR) = bepaalOmtrek(rij,kolom,rijU,rijD,kolomL,kolomR)
    toonPainting(panel,rijU,rijD,kolomL,kolomR)

def toonPainting(panel,rijU,rijD,kolomL,kolomR):
    rijen = rijD - rijU + 1
    kolommen = kolomR - kolomL + 1
    painting = ""
    for rij in range(rijen):
        for kolom in range(kolommen):
            painting = painting + panel[rij+rijU][kolom+kolomL]
        print(painting)
        painting = ""

def bepaalOmtrek(rij,kolom,rijU,rijD,kolomL,kolomR):
    if rij < rijU:
        rijU = rij
    if rij > rijD:
        rijD = rij
    ...
    if kolom > kolomR:
        kolomR = kolom
    return (rijU,rijD,kolomL,kolomR)
