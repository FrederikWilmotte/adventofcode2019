...
# We kunnen niet negatief gaan in een matrix, dus we bepalen ons "nulpunt" op matrix[20000][20000]
maxrijen = 40000
maxkolommen = 40000
kortsteafstand = 9999999
lengtewire1 = 0
lengtewire2 = 0
matrix = [[0 for x in range(maxkolommen)] for y in range(maxrijen)]
# Opvullen van de matrix
rij = 20000
kolom = 20000
for actie in wire1:
    richting = actie[0][:1]
    hoeveelheid = int(actie[1:])
    teller = 1
    while teller <= hoeveelheid:
        lengtewire1 = lengtewire1 + 1
        if richting == "R":
            kolom = kolom + 1
        elif richting == "L":
            kolom = kolom - 1
            ...
            rij = rij - 1
        matrix[rij][kolom] = lengtewire1
        teller = teller + 1
# Zoeken van kruispunten in de matrix
rij = 20000
kolom = 20000
for actie in wire2:
    richting = actie[0][:1]
    ...
    while teller <= hoeveelheid:
        ...
        if matrix[rij][kolom] > 0:
            ...
            print("Rij:",rij)
            print("Kolom:",kolom)
            afstand = lengtewire2 + matrix[rij][kolom]
            if afstand < kortsteafstand:
                print("Afstand:",afstand)
                kortsteafstand = afstand
        teller = teller + 1
print("Kortste afstand:",kortsteafstand)